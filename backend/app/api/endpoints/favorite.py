...
@router.get(
    "/favorites/", response_model=list[FavoriteBase], response_model_exclude_none=True
)
async def get_favorite(session: AsyncSession = Depends(get_async_session)):
...
@router.delete("/recipes/{recipe_id}/favorite/", response_model_exclude_none=True)
async def delete_favorite(
    recipe_id: int,
    user: User = Depends(current_user),
    session: AsyncSession = Depends(get_async_session),
):
    favorite = await check_favorite_exist(user.id, recipe_id, session, exist=True)
