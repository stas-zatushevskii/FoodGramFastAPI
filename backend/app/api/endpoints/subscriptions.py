...
# поменять return
@router.post("/users/{author_id}/subscribe/")
async def follow(
    author_id: int,
    recipes_limit: Optional[int] = 6,
    session: AsyncSession = Depends(get_async_session),
    ...
    user_db = await get_follows_with_param(
        user_id=user.id, authors=author, recipes_limit=recipes_limit, session=session
    )
    return user_db


@router.delete("/users/{author_id}/subscribe/")
async def unfollow(
    recipe_id: int,
    session: AsyncSession = Depends(get_async_session),
    user: User = Depends(current_user),
