...
class CRUDShoppingList(CRUDBase):
    ...
    async def get_lists(self, user_id: int, session: AsyncSession):
        lists = await session.execute(
            select(self.model).where(self.model.user == user_id)
        )
        return lists.scalars().all()

    async def get_recipes(self, user_id, session: AsyncSession):
        recipes_id_subquery = await session.execute(
            select(self.model.recipe).where(self.model.user == user_id)
        )
        recipes = await session.execute(
            select(Recipe)
            .options(selectinload(Recipe.ingredients))
            .where(Recipe.id.in_(recipes_id_subquery.scalars().all()))
        )
