...
class CRUDIngredient(CRUDBase):
    async def get_ingredient_by_name(self, ingredient_name: str, session: AsyncSession):
    ...
    async def add_ingredient(
        self, ingredient_id: int, recipe_id: int, amount: int, session: AsyncSession
    ):
        data = {
            "recipe_id": recipe_id,
    ...
    async def delete_from_recipe(
        self, ingredient_id: int, recipe_id: int, session: AsyncSession
    ):
        ingredient = await session.execute(
    ...
    async def ingredints_update(
        ...
        for ingredient_id in ingredients_delete:
            await self.delete_from_recipe(ingredient_id, recipe_id, session)
        # создание
        for ingredient_obj in ingredients_update:
            await self.add_ingredient(
                ingredient_obj["id"], recipe_id, ingredient_obj["amount"], session
            )
