...
class CRUDFollow(CRUDBase):
    async def create(self, user_id: int, author_id: int, session: AsyncSession):
        follow_in_data = {"author_id": author_id, "follower_id": user_id}
    ...
    async def get_follows(
        ...
        authors = await session.execute(
            select(self.model).where(self.model.id.in_(followings.scalars().all()))
        )
        return authors.scalars().all()

    async def count_follows(self, user_id: int, session: AsyncSession):
        followings = await session.execute(
            select(func.count(followers.c.author_id)).where(
    ...
    async def get_is_subscribed(
        self, user_id: int, author_id: int, session: AsyncSession
    ):
    ...
    async def get_follow(self, user_id: int, author_id: int, session: AsyncSession):
    ...
    async def get_follows_with_param(
        self, user_id: int, authors: list[User], session: AsyncSession
    ):
        results = []
        for author in authors:
            ...
            author_db = recipe_crud.count_recipes(session=session, author_id=author.id)
            results.append(author_db)
        return results

    async def delete(self, user_id: int, author_id: int, session: AsyncSession):
        follow = delete(followers).where(
            and_(followers.c.author_id == author_id, followers.c.follower_id == user_id)
        )
