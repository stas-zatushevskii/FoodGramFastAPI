...
class CRUDRecipe(CRUDBase):
    ...
    async def count_recipes(
        ...
        if is_shopping_card is True:
            count = await session.execute(
                select(func.count(ShoppingList.id)).where(ShoppingList.user == user_id)
            )
            return count.scalar()
        elif favorite is True:
            count = await session.execute(
                select(func.count(Favorite.id)).where(Favorite.user == user_id)
            )
            return count.scalar()
            ...
            count = await session.execute(select(func.count(self.model.id)))
            return count.scalar()
    ...
    async def get_paginated_recipes(
        ...
        db_objs = await session.execute(select(self.model).offset(start).limit(limit))
    ...
    async def get_recipes_by_relation(
        self, session: AsyncSession, user_id: int, related_model
    ):
        recipes = await session.execute(
            select(self.model).join(related_model).where(related_model.user == user_id)
        )
        return recipes.scalar()

    async def get_recipes_by_tag(self, session: AsyncSession, tags: list[str]):
        recipes = await session.execute(
            select(self.model).join(RecipesTags).join(Tag).filter(Tag.slug.in_(tags))
        )
        return recipes

    async def get_by_author(
        self, session: AsyncSession, author_id: int, limit: Optional[int] = None
    ):
        if limit is None:
            recipes = await session.execute(
            ...
            recipes = await session.execute(
                select(self.model).where(self.model.author_id == author_id).limit(limit)
            )
            return recipes.scalars().all()
    ...
    async def get_recipes_with_param(
        ...
        if author_id:
            ...
            # [author_id]
            author_id = author_id if isinstance(author_id, list) else [author_id]
            recipes = recipes.filter(self.model.author_id.in_(author_id))

        if tags:
            recipes = recipes.join(RecipesTags).join(Tag).filter(Tag.slug.in_(tags))

        if is_favorited:
            user_id = user_id if isinstance(user_id, list) else [user_id]
            recipes = recipes.join(Favorite).filter(Favorite.user.in_(user_id))

        if is_in_shopping_cart:
            user_id = user_id if isinstance(user_id, list) else [author_id]
            recipes = recipes.join(ShoppingList).filter(ShoppingList.user.in_(user_id))

        result = await session.execute(recipes.offset(start).limit(limit).distinct())
        recipe = result.scalars().all()
        return recipe

    async def update(
        self, db_obj: RecipeCreate, in_obj: RecipeUpdate, session: AsyncSession
    ):
        ...
        for field in update_data:
            if field in obj_data:
                if field == "ingredients":
                    ...
                    ingredients_delete = set(ingredients_old) - set(ingredients_new)
                    ingredients_add = set(ingredients_new) - set(ingredients_old)
                    ingredients_add_obj = [
                        ingredient
                        for ingredient in update_data[field]
                        if ingredient["id"] in ingredients_add
                    ]
                    if ingredients_add_obj or ingredients_delete:
                        await ingredient_crud.ingredints_update(
                            ingredients_delete, ingredients_add_obj, db_obj.id, session
                        )
                    continue
                elif field == "tags":
                    # crud function to update tags in recipe
                    tags_old = [tag["id"] for tag in obj_data[field]]
                    tags_new = [tag for tag in update_data[field]]
                    tags_delete = set(tags_old) - set(tags_new)
                    tags_add = set(tags_new) - set(tags_old)
                    if tags_add or tags_delete:
                        await tag_crud.tags_update(
                            tags_delete, tags_add, db_obj.id, session
                        )
                    continue
                elif field == "image":
